...
class RopeBuffer
	...
	private fun dump_buffer do
		written = false
		var nstr = new FlatString.with_infos(ns, rpos - dumped)
		str += nstr
		var bs = buf_size
		bs = bs * 2
	...
	redef fun output do
		str.output
		new FlatString.with_infos(ns, rpos - dumped).output
	end
	...
	redef fun reverse do
		...
		if rpos > 0 and dumped != rpos then
			str += new FlatString.with_infos(ns, rpos - dumped)
			dumped = rpos
		end
		str = str.reversed
	end
...
private class RopeBufSubstringIterator
	...
	init from(str: RopeBuffer) do
		...
		nsstr = new FlatString.with_infos(str.ns, str.rpos - str.dumped)
