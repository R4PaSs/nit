...
redef class FlatText
	...
	do
		...
		var s = new FlatString.with_infos(nits, nlen)
		return s
	end
...
class FlatString
	...
	redef fun substring_from(from) do
		...
		var fln = bytelen - c
		var nns = new NativeString(fln)
		_items.copy_to(nns, fln, c, 0)
		return new FlatString.full(nns, fln, _length - from)
	end
	...
	private fun substring_impl(from, count, end_index: Int): String do
		...
		var nlen = byteto - bytefrom + 1
		var nns = new NativeString(nlen)
		_items.copy_to(nns, nlen, bytefrom, 0)

		return new FlatString.full(nns, nlen, count)
	end
	...
	do
		...
		_bytepos = 0
		_length = _items.utf8_length(0, bytelen)
	end
	...
	redef fun +(o) do
		...
		if s isa FlatText then
			...
			return new FlatString.full(ns, nlen, _length + o.length)
		else
			abort
		end
	end
...
private class FlatStringByteView
	...
	redef fun [](index)
	do
		# Check that the index (+ 0) is not larger than last_byte
...
class FlatBuffer
	...
	do
		...
		return new FlatString.full(_items, bln, _length)
	end
	...
	do
		var bln = _bytelen
		var x = new FlatString.full(_items, bln, _length)
...
redef class NativeString
	...
	redef fun to_s_unsafe(len) do
		...
		return new FlatString.with_infos(self, len)
	end
	...
	fun clean_utf8(len: Int): FlatString do
		...
		return new FlatString.full(ret, end_length, chr_ln)
	end
...
redef class Array[E]
	...
	do
		...
		return new FlatString.with_infos(ns, sl)
	end
end
