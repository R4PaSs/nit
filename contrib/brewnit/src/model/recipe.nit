...
class Recipe
	...
	do
		...
		return new Liter((shrinkage.value / (1.0 - (equipment.boil_loss_rate * boil_length.to_h.value))) + hop_loss.to_l.value)
	end
	...
	do
		...
		for i in hops do
			if i isa Boil then
				final_ibu += (i.quantity.to_oz.value * i.use_factor(boil_grav) * (i.alpha_acid / 100.0) * 7.489) / (fermenter_volume.to_us_gal.value * correction_factor)
			end
		end
...
class Equipment
	...
	# Rate at which the liquid evaporates when boiled
	# Defaults to 5%
	var boil_loss_rate: Float = .05 is writable
end
