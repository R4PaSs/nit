...
class FlatString
	...
	# Cache of the latest position explored in the string
	private var cache = new CharCache(-1, -1) is lazy
	...
	private fun char_to_byte_index(index: Int): Int do
		...
		if min == delta_begin then
			...
			ns_i = items.find_beginning_of_char_at(last_byte)
			my_i = length - 1
		end

		ns_i = items.char_to_byte_index_cached(index, my_i, ns_i)
	...
	do
		...
		if (count + from) > length then count -= from
		var end_index = from + count - 1

		var bytefrom = char_to_byte_index(from)
		var byteto = char_to_byte_index(end_index)
		byteto += self[end_index].u8char_len - 1

		return new FlatString.full(items, byteto - bytefrom + 1, bytefrom, byteto, count)
	end
...
private class FlatStringByteIterator
	...
	do
		target = tgt
		target_items = tgt.items
		curr_pos = pos + target.first_byte
	end
...
private class FlatStringByteView
	...
	redef fun [](index)
	do
		# Check that the index (+ first_byte) is not larger than indexTo
	...
	private fun native_set_char(pos: Int, c: Char, ln: Int) `{
		...
		switch(ln){
			...
			case 2:
				...
				break;
			case 3:
				r[0] = 0xE0 | ((c & 0xF000) >> 12);
				r[1] = 0x80 | ((c & 0xFC0) >> 6);
				r[2] = 0x80 | (c & 0x3F);
				break;
			case 4:
				r[0] = 0xF0 | ((c & 0x1C0000) >> 18);
				r[1] = 0x80 | ((c & 0x3F000) >> 12);
				r[2] = 0x80 | ((c & 0xFC0) >> 6);
				r[3] = 0x80 | (c & 0x3F);
				break;
		}
		memmove(self + pos, r, ln);
	`}
end
