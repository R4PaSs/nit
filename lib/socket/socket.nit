...
class UDPSocket
	...
	redef fun read_bytes(bytes, max)
	do
		var len = native.recvfrom(bytes, max, 0, new NativeSocketAddrIn.nul)
		if len == -1 then
	...
	#
	# On error, returns an empty string and sets `error` appropriately.
	fun recv_from(bytes: NativeString, length: Int, sender: Ref[nullable SocketAddress]): Int
	do
		var src = new NativeSocketAddrIn

		var len = native.recvfrom(bytes, length, 0, src)
		if len == -1 then
			error = new IOError.from_errno
			src.free
			return 0
	...
	#
	# On error, sets `error` appropriately.
	fun send_to(dest_address: Text, port: Int, data: Bytes)
	do
		...
		if native.sendto(data.items, data.length, 0, dest) == -1 then
			error = new IOError.from_errno
		end
		dest.free
	end
	...
	fun broadcast(port: Int, data: Bytes)
	do
		var addr_in = new NativeSocketAddrIn
		addr_in.port = port
		addr_in.family = new NativeSocketAddressFamilies.af_inet
		...
		if native.sendto(data.items, data.length, 0, addr_in) == -1 then
			error = new IOError.from_errno
		end
