...
class TCPServer
	...
	do
		# We use the opposite from the native version as then native API
...
class SocketSet
	...
	fun add(socket: TCPSocket) do native.set(socket.socket)

	# Remove `socket` from this set
	fun remove(socket: TCPSocket) do native.clear(socket.socket)

	# Does this set has `socket`?
	fun has(socket: TCPSocket): Bool do return native.is_set(socket.socket)
