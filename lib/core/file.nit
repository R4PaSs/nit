...
class FileStream
	...
	redef fun close do
		var f = _file
		if f == null then
			last_error = new IOError("Cannot close non-existing file handler")
			return
		end
		f.io_close
		_file = null
...
class FileReader
	...
	redef fun read_bytes(ns, l) do
		var sup = super
		if sup >= l then return sup
		var f = _file
		if f == null then
			if sup != 0 then return sup
			last_error = new IOError("Cannot read a byte from a closed file descriptor")
			return 0
		end
		var r = f.io_read(ns.fast_cstring(sup), l - sup)
		if r <= 0 then eof = true
		return r
	end
...
class Path
	...
	fun copy(dest: Path)
	do
		var tmp = once new NativeString(1024)
		last_error = null
		var input = open_ro
		var output = dest.open_wo

		while not input.eof do
			var rd = input.read_bytes(tmp, 1024)
...
redef class Text
	...
	do
		for i in substrings do s.write_bytes(i.fast_cstring, i.bytelen)
	end
end
...
redef class FlatString
	...
	do
		s.write_bytes(items.fast_cstring(first_byte), bytelen)
	end
...
redef class Sys
	...
	# Standard input
	var stdin: CharReader = new Stdin.default is protected writable, lazy

	# Standard output
	var stdout: CharWriter = new Stdout.default is protected writable, lazy

	# Standard output for errors
	var stderr: CharWriter = new Stderr.default is protected writable, lazy
