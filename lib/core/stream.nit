...
interface Writable
	...
	fun write_to(stream: CharWriter) is abstract
...
class MemoryWriter
	...
	init from(t: Text) do
		src = new Bytes.with_capacity(t.bytelen)
		src.append_text(t)
	end
...
class MemoryReader
	...
	init from(s: Text) do init(s.bytes)
